...
namespace ParsLinks.Shared.PagedCollections;
...
/// </summary>
/// <typeparam name="T">The type of the data to page</typeparam>
public class VirtualizedList<T> : IVirtualizedList<T>
{
    /// <summary>
    /// Gets or sets the start index.
    /// </summary>
    /// <value>The started index.</value>
    ...
    /// <param name="pageSize">The size of the page.</param>
    /// <param name="indexFrom">The index from.</param>
    public VirtualizedList(IEnumerable<T> source, int startIndex, int size)
    {
        ...
        if (source is IQueryable<T> querable)
        {
            ...
            Items = querable.Skip(StartIndex).Take(Size).ToList();
        }
        else
        {
            StartIndex = startIndex;
            ...
            Items = source.Skip(startIndex).Take(Size).ToList();
        }
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="VirtualizedList{T}" /> class.
    /// </summary>
    public VirtualizedList() => Items = new T[0];
}
